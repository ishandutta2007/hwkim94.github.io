...
A&+& \lambda_{coord} \sum_{i}^{S^2} \sum_{j}^{B} \theta_{i,j}^{obj} [(\sqrt{w_i} - \sqrt{\hat{w_i}})^2 + (\sqrt{h_i} - \sqrt{\hat{h_i}})^2] \\  
A&+& \sum_{i}^{S^2} \sum_{j}^{B} \theta_{i,j}^{obj} (C_{i} - \hat{C_{i}})^2 \\ 
A&+& \lambda_{no\; obj}\sum_{i}^{S^2} \sum_{j}^{B} \theta_{i,j}^{no\; obj} (C_{i} - \hat{C_{i}})^2\\ 
A&+& \sum_{i}^{S^2} \theta_{i}^{obj} \sum_{c \in class} (p_i(c) - \hat{p_i(c)})^2
\end{matrix}$$
