...
- BiDAF : Bi-Directional Attention Flow
    ...
    - context를 요약하지않고 query-aware context를 찾아내는 방식
...
- **context vector와 query를 interaction시켜, context에서 query에 대한 답이 될 수 있는 특징들을 추출한다.**
...
- similarity matrix $${S_{tj}}$$ = $$\alpha ({H_{:t}},{U_{:j}})$$ 공유, $$S \in {R{T \tims J}}$$
...
1. **Context2Query Attention**
    ...
    - $${a_{t}}$$ = $$softmax({S_{t:}}) \in {R^{J}}$$ = t-th context word와 query word의 attention weight 
    - 단, $$\sum {a_{tj}} = 1$$ for all t
    - **similarity $${S_{tj}}$$에 softmax를 취해줬으므로 유사성이 큰 weight만 값이 커지게 됨**
    - attended query vector $$\widetilde{U_{:t}}$$ = $$\sum {a_{tj}}{U_{:j}}$$, where &&{U_{:j}} \in {R^{2d \times 1}}$$, 즉
