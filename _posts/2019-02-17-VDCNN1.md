...
- **resNet과 VGG를 기반으로 모델링**
    ...
    - filter : VGG의 3-size filter을 사용하여 3-gram 역할, character level에서 연산하기 위해 size를 키우지 않음
    - 메모리 사용을 줄이는 효과
...
- filter의 크기가 작아 parameter가 적기 때문에 convolution layer를 통하여 network의 depth를 많이 늘리는 것이 가능
