...
- YOLOv2는 convolution과 pooling으로만 구성되었기 때문에 input size의 영향을 받지않기 때문에 학습시에 다양한 resolution의 image를 input으로 제공
...
- 같은 concept node에 속한 다른 class들만 사용하여 softmax를 계산하며, 전체 확률은 조건부확률을 이용하여 계산하는 방식 사용
    - 예를 들어, $$P(Yorkshire terrier)$$ = $$P(Yorkshire terrier \mid terrier)$$ $$\times$$ $$P(terrier \mid hunting dog)$$ $$\times \cdots\ times$$ $$P(mammal \mid animal)$$ $$\times$$ $$P(animal \physical object)$$
